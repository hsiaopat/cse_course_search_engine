...
def index():
	...
	if request.method == "GET": #GET request is sent when html wants some information from python backend
		return render_template("index.html", requiredDict=requiredTaken, electivesAvailable=dataout)
	...
	if request.method == "POST": #POST request is when the webpage is sending data to python backend
		...
		requiredTaken, electiveTaken = findRequired(courseInput, cseColl, requiredDict, electiveDict)
		electives = findElectives(courseInput, keywordInput, cseColl, takenDict)

	return render_template("index.html", requiredDict=requiredTaken, electivesAvailable=electives)
...
def findRequired(courseInput, cseColl, requiredDict, electiveDict):
	...
	return requiredTaken, electiveTaken
